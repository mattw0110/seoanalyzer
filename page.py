...
IMAGE_EXTENSIONS = set(['.img', '.png', '.jpg', '.jpeg',
                       '.gif', '.bmp', '.svg', '.webp', '.avif',])
...
class Page():
    ...
    def __init__(self, url='', base_domain='', analyze_headings=True, analyze_extra_tags=True):
        ...
        self.keywords = {}
        self.warnings = []
        self.translation = bytes.maketrans(punctuation.encode(
            'utf-8'), str(' ' * len(punctuation)).encode('utf-8'))
        self.links = []
    ...
    def analyze(self, raw_html=None):
        ...
        soup_lower = BeautifulSoup(
            clean_html.lower(), 'html.parser')  # .encode('utf-8')
        soup_unmodified = BeautifulSoup(
            clean_html, 'html.parser')  # .encode('utf-8')
    ...
    def sort_freq_dist(self, freqdist, limit=1):
        aux = [(freqdist[key], self.stem_to_word[key])
               for key in freqdist if freqdist[key] >= limit]
        aux.sort()
        aux.reverse()
        return aux
    ...
    def analyze_title(self):
        ...
        if length == 0:
            ...
            self.warn(
                u'Title tag is too short (less than 10 characters): {0}'.format(t))
        elif length > 70:
            self.warn(
                u'Title tag is too long (more than 70 characters): {0}'.format(t))
